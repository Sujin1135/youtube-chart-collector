...
type DailyTopShortsSong struct {
	Rank         int       `json:"rank"`
...
func GetDailyTopShortsSongExtractScript() string {
	return `
		(function() {
			try {
				const songs = [];
				...
				chartRows.forEach((row, index) => {
					try {
						...
						// Thumbnail & Video ID 추출 (Shorts도 tracks-thumbnail 클래스 사용)
						const thumbnailEl = row.querySelector('img.tracks-thumbnail#thumbnail');
						let videoId = '';
						let thumbnailUrl = '';
						...
						if (thumbnailEl) {
							...
							// endpoint 속성에서 video ID와 URL 추출
						...
						if (metricEls.length > 0) {
							...
							// "Jan 16, 2014"를 파싱 후 KST(+09:00) 기준으로 변환
						...
						// metricEls[3]: View Count (hidden일 수 있음, "<10K" 같은 형식일 수도 있음)
						if (metricEls.length > 3) {
							const viewStr = metricEls[3].textContent.trim();
							// "<10K" 같은 형식은 0으로 처리
							if (viewStr.includes('<')) {
								viewCount = 0;
							} else {
								viewCount = parseInt(viewStr.replace(/,/g, '')) || 0;
							}
						}
